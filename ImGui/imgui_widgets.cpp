...
int ImGui::ImageButtonEx2(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    bool l_button = ImGui::IsMouseDown(0);
    bool r_button = ImGui::IsMouseDown(1);
    bool m_button = ImGui::IsMouseDown(2);

    if (l_button && !r_button && !m_button)
    {
        return 0;
    }

    if (r_button && !m_button && !l_button)
    {
        return 1;
    }

    if (m_button && !r_button && !l_button)
    {
        return 2;
    }
...
int ImGui::ImageButton2(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float)frame_padding, (float)frame_padding) : g.Style.FramePadding;
    return ImageButtonEx2(id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col);
}
